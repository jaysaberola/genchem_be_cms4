...
const editor = grapesjs.init({
    ...
    canvas: {
        styles: [
            ...
            app_url + "/css/genchemph-contact-us.css?v=5",
            app_url + "/css/genchem-theme.css?v=5",
...
function initGenchemTrustBar(doc) {
    const root = doc || document;
    const trustBar = root.querySelector(".footer-stick.dark .row.gy-4");
    if (!trustBar) return;
    ...
    trustBar.querySelectorAll(".col-lg-4").forEach((col, index) => {
        const labelEl = col.querySelector("span");
        const label = (labelEl && labelEl.textContent ? labelEl.textContent.trim() : "") || (items[index] && items[index].label) || "";
        const item = items.find((entry) => entry.label === label) || items[index];
        if (!item) return;

        col.querySelectorAll("img, i, svg.genchem-trust-icon").forEach((node) => node.remove());

        if (labelEl) {
            labelEl.insertAdjacentHTML("beforebegin", item.icon);
        } else {
            col.insertAdjacentHTML("afterbegin", item.icon);
        }
    });
}
...
cmdm.add('open-assets', {
    run(editor, sender, opts = {}) {
        ...
        const isVideo =
            ...
            /video/i.test(accept) ||
            (target && typeof target.get === "function" && target.get("type") === "video");

        const lfmType = isVideo ? "Files" : "Images";
        const route_prefix = app_url + "/laravel-filemanager";

        window.open(
            route_prefix + "?type=" + lfmType,
            "FileManager",
            "width=900,height=600"
        );

        window.SetUrl = function (url) {
            if (!url || !target || typeof target.set !== "function") return;

            if (isVideo) {
                target.set("provider", "so");
                target.set("src", url);
                if (typeof opts.onSelect === "function") {
                    opts.onSelect({ get: () => url });
                }
                return;
            }
